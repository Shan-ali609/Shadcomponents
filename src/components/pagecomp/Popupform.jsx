    ...
    return (
        <>
            <div className={`${sheetcond ? "fixed inset-y-0 top-0 mt-7 right-0 bg-gray-200 bg-opacity-90 z-50":
                ...
                <div className={`${dialprop ? (accountform ? "bg-white  px-7 py-6 rounded-md shadow-md max-w-[400px] " :  "bg-white p-6 rounded-md shadow-md max-w-[450px]  "  ): "bg-white p-6 rounded-md shadow-md max-w-[600px] dark:bg-black dark:border "}`}>
                    <h2 className="text-lg font-semibold dark:text-white">{dialprop ? (accountform ? (pwdform ? "Password " : "Account ") : "Edit Profile ") : "Are you absolutely sure?"}</h2>
                    <p className={`${dialprop ? "text-[15px] text-gray-500 " : "dark:text-gray-200"}`}>
