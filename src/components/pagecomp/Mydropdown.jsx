    ...
    <div className='relative flex items-center justify-between px-9  w-full'> 
      ...
      {selectedOption === "Default" && (
              <div className='bg-black rounded-sm '>
                  <button className=' text-xs px-2  text-white py-1 flex '>open in <FaArrowUpRightDots  className='pl-1'/> </button>
                 </div>
             )}

             <div className='border py-[4px] px-1 rounded-md'>  <AiOutlineInbox className="text-md cursor-pointer dark:text-white  text-black " />  </div>
           </div> 
  

   
      {isOpen && (
        <div className={` ${cond ? "absolute z-10 top-full  bg-white border rounded-md w-44 dark:bg-black dark:text-white" : "dark:bg-black dark:text-white absolute z-10 top-full  bg-white border rounded-md w-36"} `}>
          {cond && (
            <input
              type="text"
              placeholder="Search..."
